...
signed main() {
    ...
    for (int bit=0; bit<1<<h; ++bit) {
        ...
        vvc d = c;
        vi use;
        rep(i, h) if (bit & (1<<i)) use.push_back(i);
        kk -= use.size();
        if (kk < 0) continue;

        for (auto i: use) rep(j, w) d[i][j] = '#';

        vi col_w(w, 0);
        rep(j, w) rep(i, h) if (d[i][j] == '.') ++col_w[j];
        int cnt_w = reduce(col_w.begin(), col_w.end());
        
        sort(col_w.rbegin(), col_w.rend());
        cnt_w -= reduce(col_w.begin(), col_w.begin() + kk);
        chmin(ans, cnt_w);
    }
