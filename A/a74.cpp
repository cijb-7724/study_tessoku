...
signed main() {
    ...
    vi row(n), col(n);
    rep(i, n) row[pos_row[i+1]] = i+1;
    rep(i, n) col[pos_col[i+1]] = i+1;
    ...
    repp(k, 1, n+1) {
        while (true) {
            int where = pos_row[k];
            if (where == k-1) break;
            int up_num = row[where-1];
            swap(pos_row[k], pos_row[up_num]);
            swap(row[pos_row[k]], row[pos_row[up_num]]);
            ++ans;
        }
    }
    ...
    repp(k, 1, n+1) {
        while (true) {
            int where = pos_col[k];
            if (where == k-1) break;
            int up_num = col[where-1];
            swap(pos_col[k], pos_col[up_num]);
            swap(col[pos_col[k]], col[pos_col[up_num]]);
            ++ans;
        }
    }
