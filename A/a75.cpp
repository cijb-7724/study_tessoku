...
signed main() {
    ...
    vvi dp(n+1, vi(1441, -1));
    int ans = 0;

    dp[0][0] = 0;
    rep(i, n) rep(j, 1441) {
        if (j > d[i] || j < t[i]) dp[i+1][j] = dp[i][j];
        else dp[i+1][j] = max(dp[i][j], dp[i][j-t[i]] + 1);
    }
...
/*
dp[i][j] = i問目を解答するか決めた時点で時刻がjの時，すでに解答している問題数の最大値
dp[i][j] = dp[i-1][j] where j < t[i] || j > d[i]
dp[i][j] = max(dp[i-1][j], dp[i-1][j-t[i]]+1) where t[i] <= j <= d[i]
