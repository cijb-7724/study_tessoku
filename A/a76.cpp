...
signed main() {
    ...
    rep(i, n-1) {
        int p = seg.get(i).val();
        auto itr_l = lower_bound(x.begin(), x.end(), x[i]+l);
        auto itr_r = upper_bound(x.begin(), x.end(), x[i]+r);
        --itr_r;
        int id_l = itr_l - x.begin();
        int id_r = itr_r - x.begin();
        // cout << id_l << ' ' << id_r << el;
        seg.apply(id_l, id_r+1, p);
    }
    cout << seg.prod(n-1, n).val() << el;
}
