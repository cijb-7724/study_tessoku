...
signed main() {
    ...
    int q;
    cin >> q;
    vi type(q), x(q), u(q), v(q);
    rep(i, q) {
        cin >> type[i];
        if (type[i] == 1) cin >> x[i], --x[i];
        else cin >> u[i] >> v[i], --u[i], --v[i];
    }

    //最初に運休になっている路線を求める
    vb cancelled(m, false);
    rep(i, q) if (type[i] == 1) cancelled[x[i]] = true;

    UnionFind uf(n);
    //最後の状態を構築する
    rep(i, m) {
        if (!cancelled[i] && !uf.same(a[i], b[i])) {
            uf.unite(a[i], b[i]);
        }
    }
    ...
    rrep(i, q-1, 0) {
        if (type[i] == 1) {
            ...
            if(uf.same(u[i], v[i])) ans[i] = yes;
            else ans[i] = no;
        }
    }

    rep(i, q) {
        if (type[i] == 2) cout << ans[i] << el;
    }
}
