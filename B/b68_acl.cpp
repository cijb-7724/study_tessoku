...
public:
    ...
    //DFS(Fはスタートからposに到達する過程での"残余グラフの辺の容量"の最小値)
...
signed main() {
    ...
    int psum = 0;
    rep(i, n) if (p[i] >= 0) psum += p[i];

    mf_graph<int> g(n+2);
    rep(i, n) {
        if (p[i] >= 0) {
            g.add_edge(0, i+1, 0);
            g.add_edge(i+1, n+1, p[i]);
        } else {
    ...
    rep(i, m) {
        g.add_edge(b[i], a[i], INF);
    }

    cout << psum - g.flow(0, n+1) << el;
}
