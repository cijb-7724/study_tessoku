...
signed main() {
    ...
    vs ans;
    for (char a='0'; a<='9'; ++a) for (char b='0'; b<='9'; ++b) for (char c='0'; c<='9'; ++c) for (char d='0'; d<='9'; ++d) {
        string x = {a, b, c, d};
        bool mujun = false;
        rep(i, n) {
            if (t[i] == 1) {
    ...
    if (ans.size() == 1) cout << ans[0] << el;
    else cout << "Can't Solve" << el;
}
