...
bool ok(deque<string> &mae, deque<string> &usiro) {
    if (mae.size() == usiro.size()) return true;
    if (mae.size() -1 == usiro.size()) return true;
    return false;
}
void style(deque<string> &mae, deque<string> &usiro) {
    while (mae.size() > usiro.size()) {
        if (ok(mae, usiro)) break;
        string s = mae.back();
        mae.pop_back();
        usiro.push_front(s);
    }
